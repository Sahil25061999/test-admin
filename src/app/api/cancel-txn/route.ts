...
export async function POST(request: NextRequest) {
  try {
    ...
    const { txn_id, phone_number, txn_type } = body;
    ...
    const endpoint = '/admin/v1/cancel-transaction'
    const params = { txn_id, phone_number, txn_type };

    const response = await chrysusApi.post(endpoint, params);

    return NextResponse.json(response.data);
  } catch (error: any) {
    console.error("Cancel Transaction API error:", JSON.stringify(error?.response?.data));
    return NextResponse.json(
      {
        success: false,
