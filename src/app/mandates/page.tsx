...
// import { useNavWidth } from '@/context/NavWidthContext';
...
export default function Page() {
  ...
  const fetchMandates = useCallback(async () => {
    try {
      ...
      const res = await CHRYSUS_API.get(`/mandates?${query}`);
              ...
              status:
                mandate.status === "ACTIVE" ? (
                  ...
                  >
                    ...
                    <option className=" bg-black" value={"REVOKED"}>REVOKED</option>
                  </select>
                ) : (
                  mandate.status
                ),
  ...
  const approveStatus = async () => {
    ...
    try {
      if (selectedMandateDetails.id && selectedMandateDetails.statusToSet) {
        const res = await CHRYSUS_API.post("/mandates/", {
  ...
  return (
    <>
      <h1>Mandates</h1>
      <div className="flex items-start p-4 gap-5 mb-4 mt-4 rounded-3xl border dark:border-white relative">
