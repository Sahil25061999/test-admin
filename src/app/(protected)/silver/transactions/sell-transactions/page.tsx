...
export default function Page() {
  ...
  const {
    ...
    search,
    setSearch,
  } = useFetchList({ url: "/transaction/v1/processing/txns?product_name=SILVER24&", columns });
  ...
  useEffect(() => {
    setSearch((prev) => ({ ...prev, product_name: "SILVER24" }));
  }, []);
  ...
  return (
    <>
      ...
      {selectedRowKeys && selectedRowKeys.length > 0 ? (
        ...
        <CSVLink data={selectedData.map(item=>({...item,upi:item.meta_data?.vpa,name:item.user.first_name+ " "+item.user.last_name,phone:item.user.phone_number,userid:item.user.id,email:item.user.email}))} headers={headers}><button  className=" flex items-center gap-x-1 p-4 bg-purple-500 rounded-full text-white font-semibold my-3">
